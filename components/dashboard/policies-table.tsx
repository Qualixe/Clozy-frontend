...
export function PoliciesTable({ policies }: { policies: PolicySummary[] }) {
  const router = useRouter();
  const { token } = useAuth();
  const [pendingDelete, setPendingDelete] = React.useState<PolicySummary | null>(null);
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/policies/${pendingDelete.id}`,
        {
          method: "DELETE",
      ...
      setDeleteError(
        err instanceof Error ? err.message : "Could not delete policy."
      );
    } finally {
      setDeleting(false);
    }
  ...
  if (policies.length === 0) {
    return (
      <p className="rounded-xl border border-dashed border-border py-16 text-center text-sm text-muted-foreground">
        No policy pages yet. Create one to get started.
      </p>
    );
  }
